...
function ConfigureBudgets() {
  ...
  const handleAddBudget = async () => {
    if (category && budget && selectedMonth) {
      ...
      try {
        ...
        const response = await axios.get(`https://personalbudget-server.onrender.com/check-existing-budget/${userId}/${selectedMonth}/${category}`,{
          headers:{
  ...
  const handleSaveBudgets = async () => {
    try {
      ...
      console.log(months);
      await axios.post(
        "https://personalbudget-server.onrender.com/configure-budgets",
        {
          userId,

...
function Visuals() {
  ...
  useEffect(() => {
    ...
    if (selectedMonth) {
      axios
        .get(`https://personalbudget-server.onrender.com/get-budgets/${userId}?month=${selectedMonth}`, {
          headers: {
  ...
  useEffect(()=>{
    ...
    if (selectedMonth) {
      axios
        .get(`https://personalbudget-server.onrender.com/get-expenses/${userId}?month=${selectedMonth}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
